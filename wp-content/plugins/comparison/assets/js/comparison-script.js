var BeforeAfter = (function () {
	...
	BeforeAfter.prototype.wrapHandler = function () {
		...
		container.className = 'comparison__slider';
	...
	BeforeAfter.prototype.getWrappedImage = function (image, index) {
		...
		photo.className = 'comparison__photo';
		photo.innerHTML = image;

		if(index === 0){
			photo.className += ' comparison__photo--before';
		}
		else{
			photo.className += ' comparison__photo--after';
		}
	...
	BeforeAfter.prototype.createHandler = function () {
		var beforeAfterHandler = this.getHandler().querySelector('.comparison__slider');

		var dragHandler = document.createElement('div');
		dragHandler.className = 'comparison__divider';
		dragHandler.draggable = true;
		var dragElement = document.createElement('div');
		dragElement.className = 'comparison__handle';
	...
	BeforeAfter.prototype.getImages = function () {
		...
		if(images.length === 0){
			return this.getHandler().querySelectorAll('.comparison__layer');
		}
	return images;
	};
	...
	BeforeAfter.prototype.getContainer = function () {
		return this.getHandler().querySelector('.comparison__slider');
	};

	BeforeAfter.prototype.getDragHandler = function () {
		return this.getHandler().querySelector('.comparison__divider');
	};
	...
	BeforeAfter.prototype.getPhotoBefore = function () {
		return this.getHandler().querySelector('.comparison__photo--before');
	};

	BeforeAfter.prototype.getPhotoBeforeImage = function () {
		var photoBefore = this.getPhotoBefore().querySelector('img');
		if(photoBefore === null){
			return this.getPhotoBefore().querySelector('.comparison__layer');
		}
		return photoBefore;
	};

	BeforeAfter.prototype.getImagesWidth = function () {
		return this.getHandler().querySelector('.comparison__slider').offsetWidth;
	};
...
document.addEventListener('DOMContentLoaded', function () {
	...
	const items = document.querySelectorAll('#comparisonSlider');

	items.forEach(function(item) {

		const beforeAfter = new BeforeAfter(item);
